...
**Claude Desktop** — файл `%APPDATA%\Claude\claude_desktop_config.json`:
...
> `REVROUTE_ENABLE_PARTNERS=1` нужно ставить только если вы реально пользуетесь партнёрской
> программой — иначе 10 партнёрских тулов скрыты, чтобы не засорять `tools/list`.
...
**Cursor:** тот же JSON, в `.cursor/mcp.json`.
